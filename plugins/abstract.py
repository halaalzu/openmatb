...
class AbstractPlugin:
    ...
    def update_can_receive_key(self):
        ...
        if self.paused == True or self.is_visible() == False:
            ...
            # Always allow user input, even when automaticsolver is enabled
            # This allows users to make mistakes and interact with the interface
            # while the automatic solver continues to work in the background
    ...
    def compute_next_plugin_state(self):
        ...
        if 'displayautomationstate' in self.parameters and self.parameters['displayautomationstate']:
            # If an explicit label override is provided, use it
            if 'automationlabel' in self.parameters and isinstance(self.parameters['automationlabel'], str) and len(self.parameters['automationlabel']) > 0:
                self.automode_string = self.parameters['automationlabel']
            else:
                # Default behavior (fallback): AUTO if automaticsolver is True, else MANUAL
    ...
    def do_on_key(self, keystr, state, emulate=False):   # JC: For the solver, should take an is_solver_action parameter to separate real actions of the participant
...
class BlockingPlugin(AbstractPlugin):
    ...
    def make_slide_graphs(self):
        ...
        if self.parameters['allowkeypress'] == True:
            ...
            response_text = '<center><font size=3>'+self.parameters['response']['text']+'</font></center>'
            self.add_widget(f'press_{key_name}', SimpleHTML, container=self.container,
                            draw_order=self.m_draw+1, text=response_text, wrap_width=0.3, x=0.5, y=0.05)
