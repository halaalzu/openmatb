...
class Genericscales(BlockingPlugin):
    ...
    def make_slide_graphs(self):
        super().make_slide_graphs()
         ...
         # --- CLEAR OLD WIDGETS FROM PREVIOUS QUESTIONNAIRES/SLIDES ---
        # Hide/remove previous slider widgets and their labels
        for name, widget in list(self.widgets.items()):
            # widgets are usually named like: f'{self.alias}_label_1', f'{self.alias}_slider_1'
            if name.startswith(f'{self.alias}_label_') or name.startswith(f'{self.alias}_slider_'):
                widget.hide()
                del self.widgets[name]

        # Clear logical slider registry so refresh_widgets/stop() only see current items
        self.sliders.clear()
